...
(Chapter 6). Server A is therefore presented as a *capability* of the system — fully
...
experiment chapters test whether they deliver the reliability they promise:
...
1. **Separate orchestration from execution.** The LLM decides; pinned containers and versioned
   ...
   measured, and kept out of the numerical layer, where it would be fatal.
...
3. **Offer two tool altitudes — and default to the more reviewable one.** The methylation
   ...
   (maximally constrained); this alternative is retained in the repository as a record of the
   design space explored. The trade-off between expressivity and constraint is discussed
   further in §3.3.1.
