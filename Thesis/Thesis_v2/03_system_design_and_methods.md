...
**Contributions and Pre-existing Framework.** To delineate the scope of novel contributions, the underlying **Mimosa framework**—including its planner, Quality-Diversity evolution engine, multi-source verifier, and SmolAgents/LangGraph runner (§3.5)—as well as the **Toolomics** control plane, constitute prior work developed by the host laboratory [@legrandMimosaFrameworkEvolving2026]. These are employed here as the foundational apparatus under test. The novel engineering contributions developed specifically for this research include: the implementation of **both MCP servers** (the Nextflow/methylseq runner detailed in §3.2 and the containerised R/Bioconductor methylation server with bundled annotations in §3.3); the declarative **methylation skill** (§3.4); the **goal specification** (§3.5.5); and the **epigenomics claim adapter** (§3.7). 

In addition, several upstream infrastructure and reliability patches were contributed to both Mimosa (§3.5) and Perspicacité (§3.7) to ensure experimental stability—specifically, making per-agent execution timeouts configurable for long-running workflows, resolving Python interpreter paths in the verifier harness, and repairing silent JSON claim-parsing and configuration defects in the literature verification pipeline. #review
...
Mimosa is an evolving multi-agent framework that decomposes a scientific task into a workflow of specialised agents, each of which operates in a read–decide–act loop: it reads the current state of a shared workspace, selects a next action, executes that action through a tool, observes the result, and repeats until its subtask is complete [@legrandMimosaFrameworkEvolving2026]. Actions are not free-form text but _tool calls_ directed at MCP servers; the selection and sequencing of those operations is performed by an LLM, while the operations themselves — and the reproducible environments in which they run — reside in the servers. This division constitutes the central architectural commitment of the system and the one most relevant to reliability: the non-deterministic component (an LLM choosing what to do) is deliberately separated from the components that must behave identically on every invocation (a pinned Bioconductor container, a versioned Nextflow pipeline, a fixed set of typed tools). Importantly, Mimosa decouples model selection across its pipeline stages: the meta-orchestrator that synthesises workflows, the judge that scores them, and the execution agents that carry out subtasks can each be backed by a different LLM [@legrandMimosaFrameworkEvolving2026]. In the canonical configuration, a high-capability model (Claude Opus 4.5) drives the meta-orchestrator and the judge, while a more cost-effective model (e.g., DeepSeek-V3.2) drives the execution agents — a separation that enables competitive performance at substantially lower per-task cost than monolithic frontier-model approaches [@legrandMimosaFrameworkEvolving2026]. The full account of Mimosa's five-layer architecture, including its workflow-synthesis loop and iterative refinement mechanism, is given in §3.5. #review <mark style="background: #FF5582A6;">mimosa paper'ıyla cross check yap</mark>
...
2. **Coverage → differential methylation (Server B).** The `.cov` files are loaded, filtered on coverage, normalized, and united across samples; differential testing is then performed either per-CpG (**DMCs**, differentially methylated cytosines) or over genomic tiles (**DMRs**, differentially methylated regions), using one of two engines — **methylKit** (per-CpG Fisher/logistic regression) or **DSS** (beta-binomial dispersion-shrinkage with spatial smoothing).
...
The agents and the servers do not pass large objects to one another in messages. Within the Mimosa architecture, communication happens both between agents in the workflow, and between agents and the execution environment. They communicate through three narrow channels, chosen for reproducibility and for keeping the LLMs' contexts small:

- **Shared LangGraph State (`WorkflowState`).** Within Layer 3 (Agent execution), the multi-agent workflows generated by Mimosa's orchestrator communicate with each other by appending to a shared dictionary state. This `WorkflowState` carries small, structured payloads (such as step names, success statuses, and diagnostic answers) along the edges of the LangGraph state machine, allowing an upstream solver agent to pass its results directly to a downstream verifier or diagnostician without sharing large data.
...
- **A shared workspace directory.** Both servers mount the same workspace as Mimosa's working directory. Furthermore, all agents within a workflow execute in this same workspace and have access to prior agents' artefacts. State that must persist between steps is written there as files: for example Mimosa authors R analysis scripts to disk and then asks the server to run them; intermediate methylKit objects are serialised as `.rds` files that later steps reload; figures are written to PDF devices; and an append-only `run_report.txt` records what was decided and done. The guiding rule, stated in the skill, is that _the on-disk file is the deliverable_ — the script a human would review is exactly the script that runs, with no hidden inlined code.
...
- **Cloud execution** submits the run to the **Seqera Platform** (formerly Nextflow Tower), which dispatches the work to an **AWS Batch** compute environment or a custom configured HPC on the Seqera platform. The server talks to the Seqera REST API through a small client that launches workflows, polls their status, and cancels them, reading the workspace and compute-environment identifiers and access token from environment variables.


### 3.2.3 Scope exercised in this work

Both backends are implemented, but the compute resources available for this thesis constrain which is exercised. The project workstation has **AWS or HPC access**, so the cloud backend is described as a designed capability (confirmed with the demo dataset included with nf-core by successfully performing it on Seqera and pulling the results) #review rather than an exercised path. The local backend is functional, but aligning full real human WGBS from FASTQ with nf-core/methylseq was estimated at approximately **six days per sample** on this hardware, which is infeasible for the multi-sample replications of Chapter 4. Consequently:
...
**Models.** Mimosa is multi-model by design: distinct LLMs fill distinct roles, each configured in `config.py` and routable through OpenRouter via litellm. At the time of writing, the configured defaults are a GLM-5.1 **planner**, a Claude Opus 4.5 **workflow synthesiser** (the model that writes the pipeline code), a DeepSeek-v4 **execution-agent** model, and a Qwen **judge** that writes the verifier programs; generation uses `reasoning_effort = "medium"` and `max_tokens = 8192`. These are configuration fields, not hard-coded choices — any role can be repointed at another provider — and a controlled experiment should **pin a single, fixed configuration** across its repeated runs so that observed variance is attributable to the agent's non-determinism rather than to a changing model target.

**The goal as the task contract.** Within a run, the agent is steered by a single _goal file_ — a plain-text task specification provided alongside the data. The goal is where the methylation task is made concrete: it specifies the biological analysis to perform, the execution constraints under which to perform it, the statistical methodology to follow, and the output artefacts to produce. The goal used for the experiment runs encodes the following requirements:

- **Task scope:** from Bismark `.cov` files, call differentially methylated cytosines (DMCs) and differentially methylated regions (DMRs) between two groups; read group labels from a metadata samplesheet.
- **Execution constraints:** all R code must run through the methylation MCP server, never locally; the agent must author a complete `.R` script to disk and execute it via the server; large files must be inspected with `head`/`zcat`, never opened whole.
- **Statistical methodology:** `pipeline="bismarkCoverage"`, 10× coverage filter, trim the top 0.1 %, median normalisation, an SD pre-filter, a paired test with the donor as a covariate (`overdispersion="MN"`), DMR tile width of 500 bp, and dual reporting thresholds (lenient |Δβ|≥10 %, strict ≥25 %, both q<0.05).
- **Reproducibility contract:** a `config.yaml`, an input-detection step emitting `input_manifest.json`, numbered scripts (`00_detect_inputs.py` … `04_enrichment.R`), a `run_pipeline.sh` driver, a machine-parseable `STEP_SUMMARY …` line on each script's final stdout, and an append-only `run_report.txt`.
- **Corrective instructions:** explicit affirmation that methylKit supports paired designs, enforcement of the canonical bundled BED filenames for annotation, and TSS-distance tables for the strict DMC set — each addressing a recurring agent error observed during development.

The goal specification was iteratively refined during the development phase of this work, with each revision responding to specific failure modes observed in earlier runs. The final version represents the accumulated knowledge of what an LLM agent requires to be told — and told precisely — before it analyses WGBS data reliably. #review
